...
---
 ...
 In [Part 1](../dodgeball-colors-1/index.qmd), we determined which team colors would be best to add to the upcoming dodgeball season. We wanted to select colors that would be different from those already chosen, teams with similar colors are hard to tell apart. We used LAB-Color-Distance formulas to select colors that are most distant and chose from those.
...
For this article we will score our color palettes based off of the average ∆E distance between all the colors in a set (greatest average distance as mentioned above). This maximizes the global diversity of the set - which is exactly what we're trying to tackle with the Maximum Diversity Problem. 
...
Here is the score distribution (in Heather Prism Dusty Blue):
